...
async def run_strix_audit():
    ...
    operator_token = await get_token(
        "operator1", os.environ.get("OPERATOR_PASSWORD", "")
    )
    citizen_token = await get_token("citizen1", os.environ.get("CITIZEN_PASSWORD", ""))
    ...
    async with httpx.AsyncClient() as client:
        ...
        if list_res.status_code == 403:
            print(
                "    [PASS] Citizen blocked from listing all incidents (OPA Enforced)."
            )
        else:
            print(
                f"    [FAIL] Citizen could list incidents. HTTP {list_res.status_code}"
            )
    ...
    async with httpx.AsyncClient() as client:
        admin_res = await client.get(
            f"{API_BASE}/admin", headers={"Authorization": f"Bearer {citizen_token}"}
        )
    ...
    async with httpx.AsyncClient() as client:
        ...
        tasks = [
            client.post(f"{API_BASE}/incidents", json=payload, headers=headers)
            for _ in range(50)
        ]
        ...
        if successes > 10:
            print(
                f"    [FINDING] API accepted {successes}/50 requests without rate-limiting. Vulnerable to DoS."
            )
        else:
