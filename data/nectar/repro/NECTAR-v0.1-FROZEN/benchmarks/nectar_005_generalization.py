...
def get_stim(kind):
    ...
    if kind in ("bitter", "danger", "vision"):
        name = {"bitter": "taste_bitter",
                "danger": "smell_danger",
                "vision": "vision_looming"}[kind]
...
def run_case(case):
    ...
    if noise:
        backend._pois.append(b2.PoissonInput(
            backend._neu, target_var="v", N=1, rate=0.5 * b2.Hz,
    ...
    corr = (float(np.corrcoef(mine, refs)[0, 1])
            if len(shared) > 2 and np.std(mine) > 0 and np.std(refs) > 0 else None)
...
def aggregate(first_spike_split=False):
    ...
    for case, e in results.items():
        print(f"  {case:<18} resp={e['our_responders']:>5} "
    ...
    agg = {"results": results, "measured": True, "mock": False,
           "experiment": "NECTAR-005"}
    if len(results) >= 2:
        pairs = [("A_ref", "B_bitter"), ("A_ref", "C_danger"),
                 ("A_ref", "C_novel_vision"), ("B_bitter", "C_danger")]
        disc = {}
        for a, b in pairs:
...
def run_temporal(case):
    ...
    entry = {
        ...
        "window": "first_spike<0.5s" if case == "A_window_early" else "first_spike>=0.5s",
        "n_driven": len(stim_idx),
