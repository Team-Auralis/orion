...
def _run_phase(backend, stimulus_indices, memory, label, rate=DRIVE_RATE_HZ,
               apply_learned=False):
    backend.reset()
    ...
    print(f"  [{label}] n_spiking={len(trains):>6} | "
...
def main(n_repeat: int = 1):
    ...
    print(f"[memory] KC={len(memory.kc_indices)} MBON={len(memory.mbon_indices)} "
    ...
    for rep in range(n_repeat):
        ...
        print(f"  [reward] {reward['synapses_modified']} KC->MBON synapses "
              f"strengthened (x1.5) for {len(odor_a)} driven KC")

        recall_a = _run_phase(backend, odor_a, memory, "recall_odorA_learned",
                              apply_learned=True)
        recall_b = _run_phase(backend, odor_b, memory, "recall_odorB_control",
                              apply_learned=True)
        ...
        print(f"\n  learn_factor={learn_factor:.3f} "
              f"(odorA MBON recall/baseline)")
        print(f"  control_drift={control_drift:.3f} "
              f"(odorB MBON recall/baseline)")
        print(f"  modified_synapses={n_mods}")
        print(f"  sensory_probe -> KC fired: {probe['n_active_kc']}, "
