...
try:
    import psutil
    def ram(): return round(psutil.Process().memory_info().rss / (1024 ** 3), 3)
except ImportError:
    def ram(): return None
...
def main(duration_s: float = 0.5, n_drivers: int = 8):
    print(f"[EXP-ODOR-001] hub-driver experiment | drivers={n_drivers} "
          f"duration={duration_s}s | RAM start {ram()} GB")

    backend = Brian2Backend()
    t0 = time.time()
    backend.load_connectome(CONN, {"completeness_path": COMP})
    print(f"[load] {backend._N} neurons, 15.09M edges in {time.time()-t0:.1f}s "
          f"| RAM {ram()} GB")
    ...
    print(f"[run] {duration_s}s bio in {wall:.1f}s wall "
    ...
    for idx, t in heaviest:
        print(f"  idx={idx} flywire={backend._i2flyid[idx]} spikes={len(t)} rate_hz={len(t)/duration_s:.1f}")
    print()
